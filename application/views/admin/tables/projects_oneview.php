...
$aColumns = [
    ...
    'deadline',
    'project_cost',
    //'project_cost-IFNULL((SELECT (SELECT SUM(amount) FROM ' . db_prefix() . 'invoicepaymentrecords WHERE invoiceid=' . db_prefix() . 'invoices.id) as tbls FROM '. db_prefix() . 'invoices WHERE '. db_prefix() . 'invoices.project_id= ' . db_prefix() . 'projects.id AND '. db_prefix() . 'invoices.status=2), 0) as total_paid',
...
//'(SELECT (SELECT SUM(amount) FROM ' . db_prefix() . 'invoicepaymentrecords WHERE invoiceid=' . db_prefix() . 'invoices.id) as total_paid FROM '. db_prefix() . 'invoices WHERE '. db_prefix() . 'invoices.project_id= ' . db_prefix() . 'projects.id AND '. db_prefix() . 'invoices.status=2) as total_paid',
$sIndexColumn = 'id';
...
foreach ($rResult as $aRow) {
    ...
    //$row[] = app_format_money($aRow['project_cost'], get_projects_currency($aRow['id']));

...
$additionalColumns = hooks()->apply_filters('leads_table_additional_columns_sql', [
    ...
    'status',
    'assigned',
    '(SELECT tbl_notes.dateadded FROM ' . db_prefix() . 'notes tbl_notes WHERE tbl_notes.rel_type="lead" AND tbl_notes.rel_id=' . db_prefix() . 'leads.id ORDER BY tbl_notes.dateadded DESC LIMIT 1) as last_note_updated',
    'lastname as assigned_lastname',
    '(SELECT description FROM ' . db_prefix() . 'notes tbl_notes WHERE tbl_notes.rel_type="lead" AND tbl_notes.rel_id=' . db_prefix() . 'leads.id ORDER BY tbl_notes.dateadded DESC LIMIT 1) as last_note',
...
foreach ($rResult as $aRow) {
    ...
    $row[] = $aRow['last_note'].' ('.$last_note_updated.')';

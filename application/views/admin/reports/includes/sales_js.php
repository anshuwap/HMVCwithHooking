...
<script>
 ...
 var report_payment_report_received = $('#payment-report-received-report');
 ...
 var fnServerParams = {
   ...
   "sale_agent_team": '[name="sale_agent_team"]',
 ...
 $(function() {
   $('select[name="currency"],select[name="invoice_status"],select[name="estimate_status"],select[name="sale_agent_invoices"],select[name="sale_agent_team"],select[name="sale_agent_items"],select[name="sale_agent_estimates"],select[name="payments_years"],select[name="proposals_sale_agents"],select[name="proposal_status"],select[name="credit_note_status"]').on('change', function() {
     gen_reports();
   });
   
    $('#team_report_download').on("click", function() {
 ...
 function init_report(e, type) {
   ...
   report_team.addClass('hide');
   report_estimates.addClass('hide');
   report_payments_received.addClass('hide');
   report_payment_report_received.addClass('hide');
   ...
   // Clear custom date picker
       ...
       } else if (type == 'payment-report-received') {
         report_payment_report_received.removeClass('hide');
   ...
   function team_report() {
     if ($.fn.DataTable.isDataTable('.table-team-report')) {
       $('.table-team-report').DataTable().destroy();
     }
     initDataTable('.table-team-report', admin_url + 'reports/team_report', false, false, fnServerParams, [
       [2, 'desc'],
       [0, 'desc']
       ]).column(2).visible(false, false).columns.adjust();
   }

   function download_team_reports() {
        // if ($.fn.DataTable.isDataTable('.table-team-report')) {
        //   $('.table-team-report').DataTable().destroy();
        // }
        const evar = {};
        "undefined"!=typeof csrfData&&(evar[csrfData.token_name]=csrfData.hash);
        for(var t in fnServerParams)
            evar[t]=$(fnServerParams[t]).val();

        $.post(admin_url + 'reports/team_report_download', evar)
        .done(function( data ) {
            $('.table-team-report').html(data);
        });
   }
   ...
   function payment_report_received_reports() {
    //   console.log(fnServerParams);
    //  if ($.fn.DataTable.isDataTable('.table-payment-report-received-report')) {
    //   $('.table-payment-report-received-report').DataTable().destroy();
    //  }
    // initDataTable(e,t,a,i,n,s)
    var datas = $.ajax({
        url:admin_url + 'reports/payment_report_received',
        type:"GET",
        data:function(e) {
            "undefined"!=typeof csrfData&&(e[csrfData.token_name]=csrfData.hash);
            for(var t in fnServerParams)
                e[t]=$(fnServerParams[t]).val();
        }
    });
 ...
 function items_report(){
   ...
   function gen_reports() {
     if (!$('.chart-income').hasClass('hide')) {
       ...
       invoices_report();
     } else if (!report_team.hasClass('hide')) {
       team_report();
     } else if (!report_payments_received.hasClass('hide')) {
       payments_received_reports();
     } else if (!report_payment_report_received.hasClass('hide')) {
       payment_report_received_reports();

...
<div class="panel_s no-padding mbot5">
   <div class="panel-body">
      ...
      <p>
         <a href="<?php echo admin_url('gstinvoices/list_invoices/'.$_inv->id); ?>" target="_blank"><?php echo format_invoice_number($_inv->id); ?></a> - <?php echo app_format_money($_inv->total,$_inv->currency_name); ?>
      ...
      <div class="panel-body">
            ...
            <div class="horizontal-tabs">
               <ul class="nav nav-tabs nav-tabs-horizontal mbot15" role="tablist">
                  ...
                  <li role="presentation" class="tab-separator">
                     <a href="#tab_notes" onclick="get_sales_notes(<?php echo $invoice->id; ?>,'gstinvoices'); return false" aria-controls="tab_notes" role="tab" data-toggle="tab">
         ...
         <div class="row">
            <div class="col-md-3">
               ...
               <?php if($invoice->status == Gstinvoices_model::STATUS_PARTIALLY || $invoice->status == Gstinvoices_model::STATUS_OVERDUE){
            ...
            <div class="col-md-9 _buttons">
               ...
               <div class="pull-right">
                  ...
                  <a href="<?php echo admin_url('gstinvoices/invoice/'.$invoice->id); ?>" data-toggle="tooltip" title="<?php echo _l('edit_invoice_tooltip'); ?>" class="btn btn-default btn-with-tooltip" data-placement="bottom"><i class="fa fa-pencil-square-o"></i></a>
                  <?php } ?>
                  <div class="btn-group">
                     <a href="#" class="btn btn-default dropdown-toggle" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false"><i class="fa fa-file-pdf-o"></i><?php if(is_mobile()){echo ' PDF';} ?> <span class="caret"></span></a>
                     <ul class="dropdown-menu dropdown-menu-right">
                        <li class="hidden-xs"><a href="<?php echo admin_url('gstinvoices/pdf/'.$invoice->id.'?output_type=I'); ?>"><?php echo _l('view_pdf'); ?></a></li>
                        <li class="hidden-xs"><a href="<?php echo admin_url('gstinvoices/pdf/'.$invoice->id.'?output_type=I'); ?>" target="_blank"><?php echo _l('view_pdf_in_new_window'); ?></a></li>
                        <li><a href="<?php echo admin_url('gstinvoices/pdf/'.$invoice->id); ?>"><?php echo _l('download'); ?></a></li>
                        <li>
                           <a href="<?php echo admin_url('gstinvoices/pdf/'.$invoice->id.'?print=true'); ?>" target="_blank">
                           <?php echo _l('print'); ?>
                           </a>
                        </li>
                     </ul>
                  ...
                  <span<?php if($invoice->status == Gstinvoices_model::STATUS_CANCELLED){ ?> data-toggle="tooltip" data-title="<?php echo _l('invoice_cancelled_email_disabled'); ?>"<?php } ?>>
                  <a href="#" class="invoice-send-to-client btn-with-tooltip btn btn-default<?php if($invoice->status == Gstinvoices_model::STATUS_CANCELLED){echo ' disabled';} ?>" data-toggle="tooltip" title="<?php echo $_tooltip; ?>" data-placement="bottom"><span data-toggle="tooltip" data-title="<?php echo $_tooltip_already_send; ?>"><i class="fa fa-envelope"></i></span></a>
                  </span>
                  <?php } ?>
                  <!-- Single button -->
                  <div class="btn-group">
                     ...
                     <ul class="dropdown-menu dropdown-menu-right">
                        ...
                        <li>
                           ...
                           <?php if(($invoice->status == Gstinvoices_model::STATUS_OVERDUE
                              || ($invoice->status == Gstinvoices_model::STATUS_PARTIALLY && !empty($invoice->duedate) && $invoice->duedate && date('Y-m-d') > date('Y-m-d',strtotime(to_sql_date($invoice->duedate)))))
                              && is_invoices_overdue_reminders_enabled()){ ?>
                           <a href="<?php echo admin_url('gstinvoices/send_overdue_notice/'.$invoice->id); ?>"><?php echo _l('send_overdue_notice_tooltip'); ?></a>
                           <?php } ?>
                        </li>
                        <?php if($invoice->status != Gstinvoices_model::STATUS_CANCELLED
                        ...
                        <li>
                           <a href="<?php echo admin_url('gstinvoices/copy/'.$invoice->id); ?>"><?php echo _l('invoice_copy'); ?></a>
                        </li>
                        <?php } ?>
                        <?php if($invoice->sent == 0){ ?>
                        <li>
                           <a href="<?php echo admin_url('gstinvoices/mark_as_sent/'.$invoice->id); ?>"><?php echo _l('invoice_mark_as_sent'); ?></a>
                        </li>
                        <?php } ?>
                        <?php if(has_permission('invoices','','edit') || has_permission('invoices','','create')){ ?>
                        <li>
                           <?php if($invoice->status != Gstinvoices_model::STATUS_CANCELLED
                              && $invoice->status != Gstinvoices_model::STATUS_PAID
                              && $invoice->status != Gstinvoices_model::STATUS_PARTIALLY){ ?>
                           <a href="<?php echo admin_url('gstinvoices/mark_as_cancelled/'.$invoice->id); ?>"><?php echo _l('invoice_mark_as',_l('invoice_status_cancelled')); ?></a>
                           <?php } else if($invoice->status == Gstinvoices_model::STATUS_CANCELLED) { ?>
                           <a href="<?php echo admin_url('gstinvoices/unmark_as_cancelled/'.$invoice->id); ?>"><?php echo _l('invoice_unmark_as',_l('invoice_status_cancelled')); ?></a>
                           <?php } ?>
                        </li>
                        <?php } ?>
                        <?php if(!in_array($invoice->status, array(Gstinvoices_model::STATUS_PAID, Gstinvoices_model::STATUS_CANCELLED, Gstinvoices_model::STATUS_DRAFT))
                           && has_permission('invoices','','edit')
                           && $invoice->duedate
                           && is_invoices_overdue_reminders_enabled()){ ?>
                        <li>
                           <?php if($invoice->cancel_overdue_reminders == 1) { ?>
                           <a href="<?php echo admin_url('gstinvoices/resume_overdue_reminders/'.$invoice->id); ?>"><?php echo _l('resume_overdue_reminders'); ?></a>
                           <?php } else { ?>
                           <a href="<?php echo admin_url('gstinvoices/pause_overdue_reminders/'.$invoice->id); ?>"><?php echo _l('pause_overdue_reminders'); ?></a>
                           <?php } ?>
                        </li>
                        <?php } ?>
                        <?php
                        ...
                        <li data-toggle="tooltip" data-title="<?php echo _l('delete_invoice_tooltip'); ?>">
                           <a href="<?php echo admin_url('gstinvoices/delete/'.$invoice->id); ?>" class="text-danger delete-text _delete"><?php echo _l('delete_invoice'); ?></a>
                        </li>
                        <?php } ?>
                        <?php } ?>
                     </ul>
                  ...
                  <a href="#" onclick="record_payment(<?php echo $invoice->id; ?>); return false;"  class="mleft10 pull-right btn btn-success<?php if($invoice->status == Gstinvoices_model::STATUS_PAID || $invoice->status == Gstinvoices_model::STATUS_CANCELLED){echo ' disabled';} ?>">
         ...
         <div class="tab-content">
            <div role="tabpanel" class="tab-pane active" id="tab_invoice">
               <?php if($invoice->status == Gstinvoices_model::STATUS_CANCELLED && $invoice->recurring > 0) { ?>
               ...
               <?php } ?>
               <?php $this->load->view('admin/gstinvoices/invoice_preview_html'); ?>
            </div>
            <?php if(count($invoice->payments) > 0) { ?>
            <div class="tab-pane" role="tabpanel" id="invoice_payments_received">
               <?php include_once(APPPATH . 'views/admin/gstinvoices/invoice_payments_table.php'); ?>
            </div>
            <?php } ?>
            <?php if(count($applied_credits) > 0){ ?>
            ...
            <div role="tabpanel" class="tab-pane" id="tab_child_invoices">
               ...
               <ul class="list-group">
                  ...
                  <li class="list-group-item">
                     <a href="<?php echo admin_url('gstinvoices/list_invoices/'.$recurring->id); ?>" onclick="init_invoice(<?php echo $recurring->id; ?>); return false;" target="_blank"><?php echo format_invoice_number($recurring->id); ?>
            ...
            <div role="tabpanel" class="tab-pane" id="tab_notes">
               <?php echo form_open(admin_url('gstinvoices/add_note/'.$invoice->id),array('id'=>'sales-notes','class'=>'invoice-notes-form')); ?>
...
<?php $this->load->view('admin/gst_invoice/invoice_send_to_client'); ?>

<?php defined('BASEPATH') or exit('No direct script access allowed');
   if($invoice->status == Gstinvoices_model::STATUS_DRAFT){ ?>
            ...
            <?php if($invoice->is_recurring_from == null
               ...
               <?php } else if($show_recurring_invoice_info){ ?>
                     ...
                     if($recurring_invoice->cycles == 0 || $recurring_invoice->cycles != $recurring_invoice->total_cycles){
                        echo '<span class="label label-default padding-5 mleft5"><i class="fa fa-question-circle fa-fw" data-toggle="tooltip" data-title="'._l('recurring_recreate_hour_notice',_l('gst_invoice')).'"></i> ' . _l('next_invoice_date','<b>'._d($next_date).'</b>') .'</span>';
                     }
               }
               ?>
         </div>
         <?php if($invoice->is_recurring_from != NULL){ ?>
         <?php echo '<p class="text-muted'.($show_recurring_invoice_info ? ' mtop15': '').'">'._l('invoice_recurring_from','<a href="'.admin_url('gstinvoices/list_invoices/'.$invoice->is_recurring_from).'" onclick="init_invoice('.$invoice->is_recurring_from.');return false;">'.format_invoice_number($invoice->is_recurring_from).'</a></p>'); ?>
         <?php } ?>
      </div>
      ...
      <div class="col-md-6 col-sm-6">
         <h4 class="bold">
            <?php
               ...
               ?>
            <a href="<?php echo admin_url('gstinvoices/invoice/'.$invoice->id); ?>">
         ...
         <table class="table text-right">
            <tbody>
               ...
               <tr>
                  ...
                  <td>
                     <?php echo '-' . app_format_money(sum_from_table(db_prefix().'invoicepaymentrecords',array('field'=>'amount','where'=>array('gstinvoiceid'=>$invoice->id))), $invoice->currency_name); ?>
                  </td>
               </tr>
               <?php } ?>

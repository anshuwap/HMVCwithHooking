    ...
    <p class="font-medium-xs mtop15 text-muted checklist-item-info">
        <?php
            if($list['addedfrom'] != get_staff_user_id()) {
                echo _l('task_created_by',get_staff_full_name($list['addedfrom'], true));
                if($list['dateadded']!=NULL && $list['dateadded']!="")
                    echo ' on <i title="'.$list['dateadded'].'">'.date('d M, Y', strtotime($list['dateadded'])).'</i>';
            }
            if($list['addedfrom'] != get_staff_user_id() && $list['finished'] == 1) {
                echo ' - ';
            }
            if($list['finished_from'] != get_staff_user_id() && $list['finished'] == 1){
                echo _l('task_checklist_item_completed_by',get_staff_full_name($list['finished_from'], true));
                if($list['datefinished']!=NULL && $list['datefinished']!="")
                    echo ' on <i title="'.$list['datefinished'].'">'.date('d M, Y', strtotime($list['datefinished'])).'</i>';
            }

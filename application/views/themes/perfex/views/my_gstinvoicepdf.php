...
$info_right_column .= '<span style="font-weight:bold;font-size:27px;">GST ' . _l('invoice_pdf_heading') . '</span><br />';
...
// The items table
$items = get_gstitems_table_data($invoice, 'invoice', 'pdf');
...
foreach ($items->taxes() as $tax) {
    $tbltotal .= '<tr>
    <td align="right" width="85%"><strong>' . $tax['taxname'] . '' . '</strong></td>
...
if (get_option('total_to_words_enabled') == 1) {
    ...
    $pdf->Cell(0, 0, 'In words' . ': ' . $CI->numberword->convert($invoice->total, $invoice->currency_name), 0, 1, 'C', 0, '', 0);
    // Set the font again to normal like the rest of the pdf
    $pdf->SetFont($font_name, '', $font_size);
    $pdf->Ln(4);
}

if (count($invoice->payments) > 0 && get_option('show_transactions_on_invoice_pdf') == 1 && @$show1) {
    $pdf->Ln(4);
...
$show_custom_term = 1;
if ($show_custom_term) {
    $custom_term = '<span style="color:red;">* It\'s computer generated invoice no need signature.</span>';
    $do_not_print_quote = '<span style="color:green;"> Think twice before print – Save Paper Save Trees and Save Environment.</span>';
    $bank_details = '<strong style="color:green;">Account Details:</strong><br>
Binplus Technologies Private Limited<br>
Bank Name: ICICI Limited<br>
IFSC: ICIC0000249<br>
Account no.:024905003464<br>
Account type: Current account<br>';
    $pdf->Ln(4);
    $pdf->SetFont($font_name, 'I', $font_size);
    // $pdf->Cell(0, 0, $custom_term, 0, 1, 'L', 0, '', 0);
    $pdf->writeHTMLCell('', '', '', '', $custom_term, 0, 1, false, true, 'L', true);
    // $pdf->Ln(4);
    // $pdf->SetFont($font_name, 'I', $font_size);
    // $pdf->writeHTMLCell('', '', '', '-1', $do_not_print_quote, 0, 1, false, true, 'L', true);
    // $pdf->Ln(2);
    // $pdf->SetFont($font_name, '', $font_size);
    // $pdf->writeHTMLCell('', '', '', '', $bank_details, 0, 1, false, true, 'L', true);
}
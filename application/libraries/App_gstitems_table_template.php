...
abstract class App_gstitems_table_template
{
    /**
    ...
    */
     ...
     * @return string
     */
    protected function taxes_html($item, $type='rate', $tax_type='IGST')
    {
        $itemHTML = '';
            ...
            if (count($item['taxes']) > 0) {
                foreach ($item['taxes'] as $tax) {
                    ...
                    if ((count($item['taxes']) > 1 && get_option('remove_tax_name_from_item_table') == false) || get_option('remove_tax_name_from_item_table') == false || multiple_taxes_found_for_item($item['taxes'])) {
                        ...
                        $item_tax = (int)app_format_number($tmp[1]);
                    } else {
                        $item_tax = (int)app_format_number($tax['taxrate']);
                    }
                    if($tax_type=='UGST')
                        $item_tax = $item_tax/2;
                    if($type=="rate")
     ...
     * @return string
     */
    public function tax_heading($type='')
    {
        if($type=="")
    ...
    public function set_headings($alias = '')
    {
        ...
        $this->headings['amount'] = 'Taxable value';//_l($langFrom . '_table_amount_heading', '', false);

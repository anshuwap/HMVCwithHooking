...
function format_gst_invoice_number($id)
{
    $CI = & get_instance();
    $CI->db->select('date,number,prefix,number_format')->from(db_prefix() . 'gstinvoices')->where('id', $id);
    $invoice = $CI->db->get()->row();
...
function load_invoices_total_template()
{
    ...
    $data['total_pay_result'] = $CI->invoices_model->get_invoices_total_with_payments($_data);
...
function invoiceByProjectsId($data) {
    $CI = &get_instance();
    $invoice = $CI->invoices_model->get_invoices_by_projects($data);
    return $invoice;
}

...
function data_tables_init($aColumns, $sIndexColumn, $sTable, $join = [], $where = [], $additionalSelect = [], $sGroupBy = '', $searchAs = [])
{
    ...
    //(SELECT (SELECT SUM(amount) FROM tblinvoicepaymentrecords WHERE invoiceid=tblinvoices.id) as tbls FROM tblinvoices WHERE tblinvoices.project_id= tblprojects.id AND tblinvoices.status=2)

...
function get_gstitems_table_data($transaction, $type, $for = 'html', $admin_preview = false)
{
    include_once(APPPATH . 'libraries/App_gstitems_table.php');
    $class = new App_gstitems_table($transaction, $type, $for, $admin_preview);

    $class = hooks()->apply_filters('items_table_class', $class, $transaction, $type, $for, $admin_preview);

    if (!$class instanceof App_gstitems_table_template) {
        // show_error(get_class($class) . ' must be instance of "App_items_template"');
        show_error(get_class($class) . ' must be instance of "App_gstitems_template"');
    }
...
function sales_number_format($number, $format, $applied_prefix, $date)
{
    ...
    if ($format == 1) {
        ...
        $financial_year = (date('m', strtotime($date)) > 3) ? date('Y')."-".(date('y') +1) : (date('Y') -1)."-".date('y');
        ...
        $number = $applied_prefix . $financial_year . '/' . str_pad($number, $prefixPadding, '0', STR_PAD_LEFT);
    }

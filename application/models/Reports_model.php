...
class Reports_model extends App_Model
{
    ...
    private function get_where_report_period($field = 'date', $post=array())
    {
        $months_report      = $post['report_months'];
        $custom_date_select = '';
        if ($months_report != '') {
            if (is_numeric($months_report)) {
                // Last month
                ...
                $from_date = to_sql_date($post['report_from']);
                $to_date   = to_sql_date($post['report_to']);
                if ($from_date == $to_date) {
    ...
    public function payment_report_received($post=array())
    {
        ...
        $custom_date_select = $this->get_where_report_period(db_prefix() . 'invoicepaymentrecords.date', $post);
    ...
    public function payment_report_paid($post=array())
    {
        $this->load->model('currencies_model');
        $this->load->model('payment_modes_model');
        $select           = [
        ...
        $custom_date_select = $this->get_where_report_period('date', $post);
        if ($custom_date_select != '') {
            array_push($where, $custom_date_select);
        }

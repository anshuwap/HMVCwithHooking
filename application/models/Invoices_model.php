...
class Invoices_model extends App_Model
{
    ...
    public function get_invoices_total_with_payments($data)
    {
        ...
        $total_pay_query = 'SELECT SUM(inv_pay.amount) as total FROM ' . db_prefix() . 'invoicepaymentrecords inv_pay JOIN ' . db_prefix() . 'invoices invoices ON invoices.id=inv_pay.invoiceid WHERE '.$where_inv_pay;

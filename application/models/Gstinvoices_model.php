...
class Gstinvoices_model extends App_Model
{
    ...
    {
        return $this->db->query('SELECT DISTINCT(sale_agent) as sale_agent, CONCAT(firstname, \' \', lastname) as full_name FROM ' . db_prefix() . 'gstinvoices JOIN ' . db_prefix() . 'staff ON ' . db_prefix() . 'staff.staffid=' . db_prefix() . 'gstinvoices.sale_agent WHERE sale_agent != 0')->result_array();
    }
    ...
    {
        $this->db->select('*, ' . db_prefix() . 'currencies.id as currencyid, ' . db_prefix() . 'gstinvoices.id as id, ' . db_prefix() . 'currencies.name as currency_name');
        $this->db->from(db_prefix() . 'gstinvoices');
        $this->db->join(db_prefix() . 'currencies', '' . db_prefix() . 'currencies.id = ' . db_prefix() . 'gstinvoices.currency', 'left');
        $this->db->where($where);
        if (is_numeric($id)) {
            $this->db->where(db_prefix() . 'gstinvoices' . '.id', $id);
            $invoice = $this->db->get()->row();
            if ($invoice) {
                $invoice->total_left_to_pay = get_invoice_total_left_to_pay($invoice->invoice_number, $invoice->total);

                // $invoice->items       = get_items_by_type('invoice', $id);
                // $invoice->attachments = $this->get_attachments($id);
                $invoice->items       = get_items_by_type('invoice', $invoice->invoice_number);
                $invoice->attachments = $this->get_attachments($invoice->invoice_number);
                ...
                $invoice->payments = $this->payments_model->get_invoice_payments($invoice->invoice_number);
            }
    ...
    public function get_invoice_recurring_invoices($id)
    {
        ...
        $invoices           = $this->db->get(db_prefix() . 'gstinvoices')->result_array();
        $recurring_invoices = [];
    ...
    public function get_invoices_total($data)
    {
        ...
        for ($i = 1; $i <= 3; $i++) {
            ...
            if ($i == 1) {
                ...
                $select .= ',(SELECT SUM(amount) FROM ' . db_prefix() . 'invoicepaymentrecords WHERE gstinvoiceid=' . db_prefix() . 'gstinvoices.id) as total_paid';
            }
    ...
    public function get_invoices_total_with_payments($data)
    {
        $this->load->model('currencies_model');
        ...
        for ($i = 1; $i <= 3; $i++) {
            ...
            $where_inv_pay = " MONTH(inv_pay.date) = ".date('m')." AND YEAR(inv_pay.date) = ".date('Y');
            $pay_session = "This Month Payments";
            if (isset($data['years']) && count($data['years']) > 0) {
                $where_inv_pay = " YEAR(inv_pay.date) IN (".implode(',', $data['years']).")";
                $pay_session = "Current Payment";
            }
        }

        if (isset($data['project_id']) && $data['project_id'] != '') {
            $where_inv_pay .= " AND project_id = ".$data['project_id']." ";
        } elseif (isset($data['customer_id']) && $data['customer_id'] != '') {
            $where_inv_pay .= " AND clientid = ".$data['customer_id']." ";
        }
        $total_pay_query = 'SELECT SUM(inv_pay.amount) as total FROM ' . db_prefix() . 'invoicepaymentrecords inv_pay JOIN ' . db_prefix() . 'gstinvoices invoices ON gstinvoices.id=inv_pay.gstinvoiceid WHERE '.$where_inv_pay;
        $total_pay = $this->db->query($total_pay_query)->row()->total;
        $currency             = get_currency($currencyid);
        $result['due']        = array_sum($result['due']);
        $result['paid']       = $total_pay;
        $result['overdue']    = array_sum($result['overdue']);
        $result['currency']   = $currency;
        $result['currencyid'] = $currencyid;
        $result['pay_session']= $pay_session;

        return $result;
    }

    public function get_invoices_by_projects($data)
    {
        $this->load->model('currencies_model');
        ...
        if(isset($i)) {
            ...
            foreach ($invoices as $invoice) {
                if ($i == 1) {
                    $result[] = $invoice['outstanding'];
                } elseif ($i == 2) {
                    $result[] = $invoice['total_paid'];
                } elseif ($i == 3) {
    ...
    public function add($data, $expense = false)
    {
        ...
        $this->db->insert(db_prefix() . 'gstinvoices', $data);
        $insert_id = $this->db->insert_id();
        if ($insert_id) {

            // Update next invoice number in settings
            $this->db->where('name', 'next_gst_invoice_number');
            ...
            update_sales_total_tax_column($insert_id, 'invoice', db_prefix() . 'gstinvoices');
    ...
    public function check_for_merge_invoice($client_id, $current_invoice = '')
    {
        if ($current_invoice != '') {
            ...
            $row = $this->db->get(db_prefix() . 'gstinvoices')->row();
    ...
    public function copy($id)
    {
        ...
        if($_invoice->gst_inv_number!="")
            return false;
        $new_invoice_data             = [];
        $new_invoice_data['clientid'] = $_invoice->clientid;
        $new_invoice_data['number']   = get_option('next_gst_invoice_number');
        $this->db->update(db_prefix() . 'invoices', ['gst_inv_number' => $new_invoice_data['number']], ['id' => $id]);
        $new_invoice_data['invoice_number']   = $id;
        ...
        $id = $this->gstinvoices_model->add($new_invoice_data);
        if ($id) {
            $this->db->where('id', $id);
            $this->db->update(db_prefix() . 'gstinvoices', [
            ...
            log_activity('Generated GST Invoice ' . format_invoice_number($_invoice->id));
    ...
    public function update($data, $id)
    {
        ...
        $this->db->where('id', $id);
        $this->db->update(db_prefix() . 'gstinvoices', $data);
        ...
        if ($affectedRows > 0) {
            update_sales_total_tax_column($id, 'invoice', db_prefix() . 'gstinvoices');
    ...
    public function delete($id, $simpleDelete = false)
    {
        ...
        $this->db->where('id', $id);
        $this->db->delete(db_prefix() . 'gstinvoices');
        if ($this->db->affected_rows() > 0) {
            if (get_option('invoice_number_decrement_on_delete') == 1 && $simpleDelete == false) {
                $current_next_invoice_number = get_option('next_gst_invoice_number');
    ...
    {
        return $this->db->query('SELECT DISTINCT(YEAR(date)) as year FROM ' . db_prefix() . 'gstinvoices ORDER BY year DESC')->result_array();
    }

...
class Cron_model extends App_Model
{
    ...
    public function run($manually = false)
    {
        if ($this->can_cron_run()) {
            ...
            $this->auto_retrieve_leads(); // Added for IndiaMart Leads
    ...
    private function auto_retrieve_leads()
    {
        $auto_retrieve_leads_after = get_option('auto_retrieve_leads_after');
        $auto_retrieve_leads_last_cron = get_option('auto_retrieve_leads_last_cron');
        $default = 30;
        $min_time = 16;
        $cron_status = "failed";
        if ($auto_retrieve_leads_after == -1) {
            return;
        }
        if ($auto_retrieve_leads_after == 0) {
            $auto_retrieve_leads_after = $default;
        }
        if ($auto_retrieve_leads_after > 0 && $auto_retrieve_leads_after < $min_time) {
            $auto_retrieve_leads_after = $min_time;
        }
        $auto_retrieve_leads_after_in_sec = ($auto_retrieve_leads_after-1) * 60;
        $this->load->model('leads_model');
        if((time()-$auto_retrieve_leads_last_cron) > $auto_retrieve_leads_after_in_sec):
            $indiamart_api_url = "https://mapi.indiamart.com/wservce/enquiry/listing/GLUSR_MOBILE/7617855680/GLUSR_MOBILE_KEY/MTYwMDI0NTI5MS43ODcyIzM4OTE0NDE4/";
            $leads_data = json_decode(file_get_contents($indiamart_api_url));
            update_option('auto_retrieve_leads_last_cron', time(), 0);
            $leads = 0;
            foreach($leads_data as $lead_data):
                if(isset($lead_data->Error_Message)) break;
            ...
            if($cron_status == "success")
                log_activity('IndiaMart CRON completed ['.$leads.' Lead Added]');
            else
                log_activity('IndiaMart CRON failed, Check Cron Time Limit or API. ['.$leads.' Lead Added]');
        endif;
    }

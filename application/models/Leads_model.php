...
class Leads_model extends App_Model
{
    ...
    public function getCronDetails($id = '', $where = [])
    {
        $this->db->select('*,' . db_prefix() . 'leads.name, ' . db_prefix() . 'leads.id,' . db_prefix() . 'leads_status.name as status_name,' . db_prefix() . 'leads_sources.name as source_name');
        $this->db->join(db_prefix() . 'leads_status', db_prefix() . 'leads_status.id=' . db_prefix() . 'leads.status', 'left');
        $this->db->join(db_prefix() . 'leads_sources', db_prefix() . 'leads_sources.id=' . db_prefix() . 'leads.source', 'left');
        $where1[db_prefix() . 'leads.phonenumber'] = $where[db_prefix() . 'leads.phonenumber'];
        unset($where[db_prefix() . 'leads.phonenumber']);
        $this->db->where($where);
        $where2[db_prefix() . 'leads.phonenumber'] = str_replace('-', '', str_replace('+91', '', $where1[db_prefix() . 'leads.phonenumber']));
        $where3[db_prefix() . 'leads.phonenumber'] = '+91'.$where2[db_prefix() . 'leads.phonenumber'];
        $where4[db_prefix() . 'leads.phonenumber'] = '+91-'.$where2[db_prefix() . 'leads.phonenumber'];
        $this->db->group_start();
    ...
    {
        $integration = (get_staff_user_id()=="")?true:false; // Added for CRON
        ...
        if ($insert_id) {
            ...
            $this->log_lead_activity($insert_id, 'not_lead_activity_created', $integration);

...
class Reports extends AdminController
{
        ...
        public function sales()
    {
        ...
        $data['notes_sale_agents'] = $this->notes_model->get_sale_agents();
    ...
    public function sale_account()
    {
        ...
        $data['title'] = _l('sales_reports');
        $this->load->view('admin/reports/sale_account', $data);
    }
    ...
    {
        // echo $this->input->post('report_months');
        if ($this->input->is_ajax_request()) {
            $this->load->model('currencies_model');
    ...
    public function payment_report_received()
    {
        // echo "payment_report_received";exit;
        // if ($this->input->is_ajax_request()) {
            $this->load->model('currencies_model');
            $this->load->model('payment_modes_model');
            $payment_gateways = $this->payment_modes_model->get_payment_gateways(true);
    ...
    private function get_where_report_period_team($fields = 'dateadded')
    {
        $field = 'DATE('.$fields.')';
    ...
    // SALES TEAM NOTES REPORT - DATE WISE.
    public function team_report()
    {
        // if ($this->input->is_ajax_request()) {
            ...
            $select = [
                db_prefix() . 'notes.description',
                db_prefix() . 'notes.dateadded',
                db_prefix() . 'leads.id as lid',
            ...
            $where = [
                'AND '. db_prefix() . 'notes.rel_type = "lead"',
                // 'AND '. db_prefix() . 'notes.rel_id = 132',
            ];
            
            if ($this->input->post('sale_agent_team')) {
                $agents  = $this->input->post('sale_agent_team');
                $_agents = [];
                if (is_array($agents)) {
                    foreach ($agents as $agent) {
                        if ($agent != '') {
                ...
                if (count($_agents) > 0) {
                    array_push($where, 'AND '. db_prefix() . 'notes.addedfrom IN (' . implode(', ', $_agents) . ')');
                }
            }
            $custom_date_select = $this->get_where_report_period_team(db_prefix() . 'notes.dateadded');
            if ($custom_date_select != '') {
                array_push($where, $custom_date_select);
            }
            $aColumns     = $select;
            ...
            $join         = [
                ...
                'LEFT JOIN ' . db_prefix() . 'leads ON ' . db_prefix() . 'leads.id = ' . db_prefix() . 'notes.rel_id',
                'LEFT JOIN ' . db_prefix() . 'leads_status ON ' . db_prefix() . 'leads_status.id = ' . db_prefix() . 'leads.status',
                'LEFT JOIN ' . db_prefix() . 'leads_status ls ON ls.id = ' . db_prefix() . 'notes.lead_current_status',
            ...
            $result = data_tables_init($aColumns, $sIndexColumn, $sTable, $join, $where, [
                ...
                'ls.color as notes_color',
                'status',
                db_prefix() . 'notes.addedfrom as assigned',
                db_prefix() . 'notes.description',
                db_prefix() . 'notes.dateadded as last_note_updated',
                'lastname as assigned_lastname',
                db_prefix() . 'notes.description as last_note',
            ...
            foreach ($rResult as $aRow) {
                ...
                $row[] = $aRow['id'];
                $hrefAttr = 'href="' . admin_url('leads/index/' . $aRow['lid']) . '" onclick="init_lead(' . $aRow['lid'] . ');return false;"';
                $nameRow = '<a ' . $hrefAttr . '>' . $aRow['name'] . '</a>';
                $nameRow .= '<div class="row-options">';
                $nameRow .= '<a ' . $hrefAttr . '>' . _l('view') . '</a>';
                $nameRow .= ' | <a href="' . admin_url('leads/index/' . $aRow['lid'] . '?edit=true') . '" onclick="init_lead(' . $aRow['lid'] . ', true);return false;">' . _l('edit') . '</a>';
                $nameRow .= '</div>';
                
                $row[] = $nameRow;
                // $row[]    = '<a ' . $hrefAttr . '>' . $aRow['lid'] . '</a>';
                // $row[]    = '#' . $aRow['lid'];
                ...
                $row[] = $aRow['description'];
                $last_note_updated = ($aRow['last_note_updated'] == '0000-00-00 00:00:00' || !is_date($aRow['last_note_updated']) ? '' : '<span data-toggle="tooltip" data-title="' . _dt($aRow['last_note_updated']) . '" class="text-has-action">' . time_ago($aRow['last_note_updated']) . '</span>');
                $row[] = $last_note_updated;
    ...
    $outputStatus1 = $outputStatus;
    if ($aRow['notes_status_name'] == null) {
        if ($aRow['lost'] == 1) {
            $outputStatus1 = '<span class="label label-danger inline-block">' . _l('lead_lost') . '</span>';
        } elseif ($aRow['junk'] == 1) {
            $outputStatus1 = '<span class="label label-warning inline-block">' . _l('lead_junk') . '</span>';
        }
    } else {
        $outputStatus1 = '<span class="inline-block label label-' . (empty($aRow['notes_color']) ? 'default': '') . '" style="color:' . $aRow['notes_color'] . ';border:1px solid ' . $aRow['notes_color'] . '">' . $aRow['notes_status_name'];
            $outputStatus1 .= '<div class="dropdown inline-block mleft5 table-export-exclude">';
            // $outputStatus1 .= '<a href="#" style="font-size:14px;vertical-align:middle;" class="dropdown-toggle text-dark" id="tableLeadsStatus-' . $aRow['id'] . '" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false">';
            $outputStatus1 .= '<span data-toggle="tooltip" title="' . _l('ticket_single_change_status') . '"></span>';
            // $outputStatus1 .= '</a>';
            $outputStatus1 .= '</div>';
        $outputStatus1 .= '</span>';
    }
    ...
    public function team_report_download()
    {
        // if ($this->input->is_ajax_request()) {
            $invoice_taxes     = $this->distinct_taxes('invoice');
            $totalTaxesColumns = count($invoice_taxes);
            ...
            $select = [
                db_prefix() . 'notes.description',
                db_prefix() . 'notes.dateadded',
                'DATE('.db_prefix() . 'notes.dateadded) as notes_date',
                db_prefix() . 'leads.id as lid',
                db_prefix() . 'leads.name as name',
                'company',
                db_prefix() . 'leads.email as email',
                ...
                db_prefix() . 'leads_sources.name as source_name',
                'lastcontact',
                db_prefix() . 'leads.dateadded as ldateadded',
            ];
            ...
            $query = "SELECT ".implode(", ", $select)." FROM ".db_prefix() . "notes ".implode(' ', $join)." WHERE 1 ".implode(' ', $where)." ORDER BY " . db_prefix() . "notes.dateadded ASC";
            $rResult = $this->db->query($query)->result();

            foreach ($rResult as $aRow) {
                if(!$dates[$aRow->notes_date])
                    $dates[$aRow->notes_date] = array();
                $atime = date('H:i:s', strtotime($aRow->dateadded));
                $time = date('h.i A', strtotime($aRow->dateadded));
                if(!$dates[$aRow->notes_date]['started'])
                    $dates[$aRow->notes_date]['started'] = $time;
                $dates[$aRow->notes_date]['end'] = $time;
                $dates[$aRow->notes_date]['snap_time'][] = $atime;
                $dates[$aRow->notes_date]['leads'][] = $aRow->lid;
                // print_r($dates[$aRow->notes_date]);
                // echo "Date: 12 July 2021: Started at: 01.57 PM Total Leads: 13 End: 5.00PM";
                // echo "<br>";
                // echo "13.57, 13.59, 14.02, 14.08, 14.31, Gap of 1 Hour 30 Minutes 16.06, 16.09, 16.11, 16.13, 16.16, 16.25, 16.31, 16.54";
                // echo "<hr>";
            }
            $allow_diff_in_seconds = 600;
            foreach($dates as $dt => $date):
                $total_gaps = 0;
                echo "<strong>Date: ".date('d M Y', strtotime($dt)).": Started at: ".$date['started']." Total Leads: ".count(array_unique($date['leads']))." End: ".$date['end']."</strong>";
                echo "<br>";
                foreach($date['snap_time'] as $i => $snap_time):
                    $time1 = new DateTime($dt.' '.$snap_time);
                    $time2 = new DateTime($dt.' '.$date['snap_time'][$i-1]);
                    $timediff = $time1->diff($time2);
                    $diffInSeconds = $time1->getTimestamp() - $time2->getTimestamp();
                    if($diffInSeconds > $allow_diff_in_seconds && $date['snap_time'][$i-1]!=""):
                        echo '<b style="color: red">'.$timediff->format(' Gap of %h Hour %i Minutes').'</b> ';
                        $total_gaps += $diffInSeconds;
                    endif;
                    echo $snap_time;
                    if(end($date['snap_time'])!=$snap_time)
                        echo ",";
                    // echo $timediff->format('%y year %m month %d days %h hour %i minute %s second')."<br/>";
                endforeach;
                // echo implode(", ", $date['snap_time']);
                // echo "13.57, 13.59, 14.02, 14.08, 14.31, Gap of 1 Hour 30 Minutes 16.06, 16.09, 16.11, 16.13, 16.16, 16.25, 16.31, 16.54";
                if($total_gaps>0)
                    echo "<br><b style='color: red'>Total Gaps: {$this->secToHR($total_gaps)}</b>";
                echo "<hr>";
            endforeach;

...
class Gstinvoices extends AdminController
{
    public function __construct()
    {
        parent::__construct();
        $this->load->model('gstinvoices_model');
    ...
    public function list_invoices($id = '')
    {
        ...
        $data['invoiceid']            = $id;
        $data['title']                = _l('gst_invoices');
        $data['invoices_years']       = $this->gstinvoices_model->get_invoices_years();
        $data['invoices_sale_agents'] = $this->gstinvoices_model->get_sale_agents();
        $data['invoices_statuses']    = $this->gstinvoices_model->get_statuses();
        $data['bodyclass']            = 'invoices-total-manual';
        $this->load->view('admin/gst_invoice/manage', $data);
    }
    ...
    public function recurring($id = '')
    {
        ...
        $data['invoices_years']       = $this->gstinvoices_model->get_invoices_years();
        $data['invoices_sale_agents'] = $this->gstinvoices_model->get_sale_agents();
        $this->load->view('admin/gst_invoice/recurring/list', $data);
    }
    ...
    public function client_change_data($customer_id, $current_invoice = '')
    {
        if ($this->input->is_ajax_request()) {
            ...
            if ($current_invoice != '') {
                ...
                $current_invoice_status = $this->db->get(db_prefix().'gstinvoices')->row()->status;
            }

            $_data['invoices_to_merge'] = !isset($current_invoice_status) || (isset($current_invoice_status) && $current_invoice_status != gstinvoices_model::STATUS_CANCELLED) ? $this->gstinvoices_model->check_for_merge_invoice($customer_id, $current_invoice) : [];

            $data['merge_info'] = $this->load->view('admin/gst_invoice/merge_invoice', $_data, true);

            $this->load->model('currencies_model');

            $__data['expenses_to_bill'] = !isset($current_invoice_status) || (isset($current_invoice_status) && $current_invoice_status != gstinvoices_model::STATUS_CANCELLED) ? $this->gstinvoices_model->get_expenses_to_bill($customer_id) : [];

            $data['expenses_bill_info'] = $this->load->view('admin/gst_invoice/bill_expenses', $__data, true);
    ...
    public function update_number_settings($id)
    {
        ...
        if (has_permission('invoices', '', 'edit')) {
            ...
            $this->db->where('id', $id);
            $this->db->update(db_prefix().'gstinvoices', [
    ...
    public function validate_invoice_number()
    {
        ...
        if (total_rows(db_prefix().'gstinvoices', [
    ...
    public function pause_overdue_reminders($id)
    {
        if (has_permission('invoices', '', 'edit')) {
            $this->db->where('id', $id);
            $this->db->update(db_prefix().'gstinvoices', ['cancel_overdue_reminders' => 1]);
        }
        redirect(admin_url('gstinvoices/list_invoices/' . $id));
    }
    ...
    public function resume_overdue_reminders($id)
    {
        if (has_permission('invoices', '', 'edit')) {
            $this->db->where('id', $id);
            $this->db->update(db_prefix().'gstinvoices', ['cancel_overdue_reminders' => 0]);
        }
        redirect(admin_url('gstinvoices/list_invoices/' . $id));
    }
    ...
    public function mark_as_cancelled($id)
    {
        if (!has_permission('invoices', '', 'edit') && !has_permission('invoices', '', 'create')) {
            access_denied('gstinvoices');
        }

        $success = $this->gstinvoices_model->mark_as_cancelled($id);
    ...
    public function unmark_as_cancelled($id)
    {
        ...
        $success = $this->gstinvoices_model->unmark_as_cancelled($id);
        if ($success) {
    ...
    {
        if (!$id) {
            redirect(admin_url('gstinvoices'));
        }
        if (!has_permission('invoices', '', 'create')) {
            access_denied('gstinvoices');
        }
        $new_id = $this->gstinvoices_model->copy($id);
        if ($new_id) {
            set_alert('success', _l('invoice_copy_success'));
            redirect(admin_url('gstinvoices/invoice/' . $new_id));
        } else {
            set_alert('success', _l('invoice_copy_fail'));
        }
        redirect(admin_url('gstinvoices/invoice/' . $id));
    }

    public function get_merge_data($id)
    {
        $invoice = $this->gstinvoices_model->get($id);
    ...
    public function invoice($id = '')
    {
        if ($this->input->post()) {
            ...
            if ($id == '') {
                ...
                $id = $this->gstinvoices_model->add($invoice_data);
                if ($id) {
                    set_alert('success', _l('added_successfully', _l('invoice')));
                    $redUrl = admin_url('gstinvoices/list_invoices/' . $id);
                ...
                $success = $this->gstinvoices_model->update($invoice_data, $id);
                if ($success) {
        ...
        if ($id == '') {
            ...
            $data['invoices_to_merge'] = $this->gstinvoices_model->check_for_merge_invoice($invoice->clientid, $invoice->id);
            $data['expenses_to_bill']  = $this->gstinvoices_model->get_expenses_to_bill($invoice->clientid);
        ...
        $this->load->view('admin/gst_invoice/invoice', $data);
    }
    ...
    public function get_invoice_data_ajax($id)
    {
        ...
        $data['invoices_to_merge']          = $this->gstinvoices_model->check_for_merge_invoice($invoice->clientid, $id);
        $data['members']                    = $this->staff_model->get('', ['active' => 1]);
        $data['payments']                   = $this->payments_model->get_invoice_payments($id);
        $data['activity']                   = $this->gstinvoices_model->get_invoice_activity($id);
        $data['totalNotes']                 = total_rows(db_prefix().'notes', ['rel_id' => $id, 'rel_type' => 'invoice']);
        $data['invoice_recurring_invoices'] = $this->gstinvoices_model->get_invoice_recurring_invoices($id);
        ...
        $this->load->view('admin/gst_invoice/invoice_preview_template', $data);
    }
    ...
    public function apply_credits($invoice_id)
    {
        ...
        redirect(admin_url('gstinvoices/list_invoices/' . $invoice_id));
    }
    ...
    public function record_invoice_payment_ajax($id)
    {
        ...
        $data['invoice']  = $this->gstinvoices_model->get($id);
        $data['payments'] = $this->payments_model->get_invoice_payments($id);
        $this->load->view('admin/gst_invoice/record_payment_template', $data);
    }
    ...
    public function record_payment()
    {
        ...
        if ($this->input->post()) {
            ...
            redirect(admin_url('gstinvoices/list_invoices/' . $this->input->post('invoiceid')));
        }
    }
    ...
    public function send_to_email($id)
    {
        ...
        try {
            $success = $this->gstinvoices_model->send_invoice_to_client($id, '', $this->input->post('attach_pdf'), $this->input->post('cc'));
        } catch (Exception $e) {
    ...
    public function delete_payment($id, $invoiceid)
    {
        ...
        redirect(admin_url('gstinvoices/list_invoices/' . $invoiceid));
    }
    ...
    public function delete($id)
    {
        ...
        if (!$id) {
            redirect(admin_url('gstinvoices/list_invoices'));
        }
        $success = $this->gstinvoices_model->delete($id);
    ...
    public function delete_attachment($id)
    {
        ...
        if ($file->staffid == get_staff_user_id() || is_admin()) {
            echo $this->gstinvoices_model->delete_attachment($id);
        } else {
    ...
    public function send_overdue_notice($id)
    {
        ...
        $send = $this->gstinvoices_model->send_invoice_overdue_notice($id);
        if ($send) {
    ...
    public function pdf($id)
    {
        ...
        $invoice_number = format_gst_invoice_number($invoice->id);
      	
      	try {
            $pdf = gstinvoice_pdf($invoice);
        } catch (Exception $e) {
            $message = $e->getMessage();
            echo $message;
            if (strpos($message, 'Unable to get the size of the image') !== false) {
    ...
    public function mark_as_sent($id)
    {
        ...
        $success = $this->gstinvoices_model->set_invoice_sent($id, true);
        if ($success) {

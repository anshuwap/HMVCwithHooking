...
class hrm_model extends App_Model
{
    ...
    public function get_job_position($id = false)
    {
        ...
        if ($id == false) {
            return $this->db->query('select * from ' . db_prefix() . 'job_position')->result_array();
        }
...
public function evalmath($equation)
{
   ...
   return $result;
    ...
    public function get_hrm_dashboard_data(){
        ...
        $staff_birthday = $this->db->query('SELECT * FROM tblstaff WHERE MONTH(birthday) = '.date('m').' AND YEAR(birthday) = '.date('Y').' ORDER BY birthday ASC')->result_array();

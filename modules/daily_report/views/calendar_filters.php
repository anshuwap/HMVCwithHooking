...
<div id="daily_report_filters" style="<?php if(!$this->input->post('calendar_filters')){ echo 'display:none;'; } ?>">
    <?php echo form_open(); ?>
    <?php echo form_hidden('calendar_filters',true); ?>
    <?php
        $staffs = array(
            (object)array("id" => get_staff_user_id(), "name" => "My Report"),
            (object)array("id" => 9, "name" => "Team Report")
        );
    ?>
    <div class="row">
        <div class="col-md-3">
            <div class="select">
                <label for="cf_staffs">Staffs</label>
                <select name="staffs" id="cf_staffs">
                    <?php foreach($staffs as $staff): ?>
                    <option value="<?= $staff->id ?>" <?php if($this->input->post('staffs') && $this->input->post('staffs')==$staff->id){echo ' selected';} ?>><?= $staff->name ?></option>
                    <?php endforeach; ?>
                </select>
            </div>
